...
def main():
    ...
    def compute_metrics(p: EvalPrediction):
        ...
        if data_args.task_name is not None:
            ...
            pred = preds #np.argmax(preds, axis=1)
            labels=p.label_ids
    ...
    def get_results(trainer, test_datasets, label_list):
        ...
        for test_dataset in test_datasets:
            ...
            for d in test_dataset:
                if(d['label']!=-1):
                    gold_labels.append(d['label'])

            predict_dataset = test_dataset.remove_columns("label")
            result = trainer.predict(predict_dataset, metric_key_prefix="predict")  # .predictions
            print(result)
            print(result.predictions)
            # predictions = np.argmax(predictions, axis=1) #np.squeeze(predictions) if is_regression else np.argmax(predictions, axis=1)
            ...
            print("gold_labels size:{}".format(len(gold_labels)))
    ...
    #
    #         output_predict_file = data_args.out_file #os.path.join(training_args.output_dir, f"predict_results_{task}.txt")
    ...
    print("Macro F1: %0.3f" % (overall_test_results["prf_macro"][2]))
